...
function initialise() {
	...
	function getButtons(){
		...
		if($(window).width() >= 650) {
			...
			buttonCode=$('input[name=agebutton_d]:checked').val();

		} else {
			buttonCode=$(".dropdown option:selected").val();;
...
//
		...
		function readingData(error,data1,data2,data3){
			...
			data3[0][buttonCode]=parseFloat(data3[0][buttonCode].replace(/,/g, ''));
...
// console.log(data1,data2,data3)
			...
			d3.select("#percentage").text(numberFormatP(+data3[0][buttonCode]))
			// get the name of LSOA including spaces
			LSOA_name=data3[0]["Name"]

			//fire the function dealing with area name displayed
				areaName();
} else {
	//strip the comma out of the data
	data1[0]["X"+buttonCode]=parseFloat(data1[0]["X"+buttonCode].replace(/,/g, ''));
	data2[0]["X"+buttonCode]=parseFloat(data2[0]["X"+buttonCode].replace(/,/g, ''));
	data3[0]["X"+buttonCode]=parseFloat(data3[0]["X"+buttonCode].replace(/,/g, ''));

// console.log(data1,data2,data3)


	//Put the data in the html
	d3.select("#spd").text(numberFormat(+data1[0]["X" +buttonCode]))

	d3.select("#sape").text(numberFormat(+data2[0]["X" +buttonCode]))

	d3.select("#percentage").text(numberFormatP(+data3[0]["X" +buttonCode]))
	// get the name of LSOA including spaces
	LSOA_name=data3[0]["Name"]
...
function addRemoveAreas(oldDistricts, newDistricts) {
	...
	function buildDataInView(d,csvData) {
			...
			flattenedData.forEach(function(d) {rateById[d.Code] = +eval("d.Diff_"+buttonCode); areaById[d.Code] = d.LA_name});
...
//					.attr("transform","translate(0,30)");
		...
		y = d3.scale.linear()
		    .domain([newbreaks[0], breaks[7]]) /*range for data*/
...
var tickLabels=["-54.7", "-35","-8.5", "-3.8", "3.8", "8.5", "35", "163.6"]
